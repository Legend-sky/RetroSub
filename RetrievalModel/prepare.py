...
def make_vocab(batch_seq, char_level=False):    #创建字典，统计各个字符出现次数
    cnt = Counter()
...
if __name__ == "__main__":
    ...
    for src_line, tgt_line in zip(open(args.train_data_src).readlines(), open(args.train_data_tgt).readlines()):
        src_line = src_line.strip().split() #strip():删除首尾空格，split():以空格进行分隔
        tgt_line = tgt_line.strip().split()
        tot_lines += 1
        if args.min_len <= len(src_line) <= args.max_len and args.min_len <= len(tgt_line) <= args.max_len: #1~1024
